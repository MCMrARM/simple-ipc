...
void rpc_handler::add_handler(std::string const& method, call_handler handler) {
...
void rpc_handler::invoke(std::string const& method, nlohmann::json const& data) {
    if (handlers.count(method) < 0)
        throw std::runtime_error("No handler available for this method");
    handlers.at(method)(method, data);
}